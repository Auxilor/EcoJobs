...
object EcoJobsJobTopPlaceholder : RegistrablePlaceholder {
    private val pattern = Pattern.compile("top_([a-z0-9_]+)_(\\d+)_(name|level|amount)")

    override fun getPattern(): Pattern = pattern
    override fun getPlugin(): EcoPlugin = com.willfp.ecojobs.plugin
    ...
    override fun getValue(params: String, ctx: PlaceholderContext): String? {
        ...
        return when (type) {
            "name" -> job.leaderboard?.getTop(place)?.player?.savedDisplayName ?: emptyPosition
            "level", "amount" -> job.leaderboard?.getTop(place)?.value?.toInt()?.toString() ?: emptyPosition
            else -> null
        }
    }
}